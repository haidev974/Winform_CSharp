...
namespace DO_AN_LT_WIN.Student
{
    public partial class frm_TrangThiGiuaKy : Form
    {
        ...
        {
            var chuongList = DBContext.Chuong
        ...
        private void SaveExamResults(Dictionary<string, string> selectedAnswers, string maSinhVien, string maKyThi, int lanThi)
        {
            string mabaithi;
            string maketqua;
            using (var context = new DBContext())
            {
                ...
                int lastkq = allMaBaiThi
                    .Select(b => int.TryParse(b.Substring(4), out int id) ? id : 0) // Chỉ phân tích những mã hợp lệ
                    .DefaultIfEmpty(0)
                    .Max();
                maketqua = "KQT" + (lastId + 1).ToString(); // Tạo mã mới
        ...
        private void btn_nopBai_Click(object sender, EventArgs e)
        {
            if (lbl_ThoiGian.Text.Contains("Hết giờ"))
            {
                ...
                // Lưu kết quả thi
                //SaveExamResults(selectedAnswers, masinhvien, makythi, 1);
