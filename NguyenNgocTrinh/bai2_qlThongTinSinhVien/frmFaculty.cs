...
namespace bai2_qlThongTinSinhVien
{
    ...
    {
        SinhVienDBContext k = new SinhVienDBContext();
        public frmFaculty()
        {
            InitializeComponent();
        }
        ...
        {
            List<Faculty> facultieList = k.Faculties.ToList();
            fillKhoa(facultieList);
        }

        private void fillKhoa(List<Faculty> facultieList)
        {
            dgvKhoa.Rows.Clear();
            foreach (Faculty item in facultieList)
            {
